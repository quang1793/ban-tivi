...
Thu_muc_Cong_ty= "ung_dung/Du_lieu/Cong_ty/"
# Xử lý Lưu trữ 
...
def Tao_Chuoi_HTML_Select_Nhom_Tivi(Danh_sach_nhom_Tivi):
    Chuoi_HTML_select = '<select name="Th_Ma_so">'
    for Nhom_Tivi in Danh_sach_nhom_Tivi:
        Chuoi_HTML_select += "<option value='"+Nhom_Tivi['Ma_so']+\
        "'>"+Nhom_Tivi['Ten']+"</option>"
    Chuoi_HTML_select += '</select>'
    return Markup(Chuoi_HTML_select)
...
def Tao_Chuoi_HTML_Thong_ke_SL_Ton_Tivi(Danh_sach_Thong_ke):
    ...
    Chuoi_HTML_Danh_sach += '<div class="table-responsive">'
    Chuoi_HTML_Danh_sach += '<table class="table table-bordered">'
    ...
    Chuoi_HTML_Danh_sach += '<tbody>'

    for Nhom_Tivi in Danh_sach_Thong_ke:
        Chuoi_HTML = '''<tr>
        <th scope="row">'''+ str(stt) +'''</th>
        <td>'''+ Nhom_Tivi['Ten']+'''</td>
        <td>'''+ str(Nhom_Tivi['So_luong_Ton']) +'''</td>
        </tr>        
        '''     
        stt += 1        
        Chuoi_HTML_Danh_sach +=Chuoi_HTML 

    Chuoi_HTML_Danh_sach += '</tbody></table></div>'               
    return Markup(Chuoi_HTML_Danh_sach) 
...
def Doc_Danh_sach_Ma_So_Hinh(Danh_sach_Tivi):
    Danh_sach_hinh = []
    for Tivi in Danh_sach_Tivi:
        So = Tivi["Ma_so"].split("_")
        Danh_sach_hinh.append(int(So[1]))
        print("Test",So[1])
    print(Danh_sach_hinh)
    return Danh_sach_hinh

def Lay_Nhom_Tivi(Nhom_Tivi, Danh_sach_nhom_Tivi):    
    Danh_sach  = list(filter(
        lambda N_Tivi: N_Tivi["Ma_so"] == Nhom_Tivi, Danh_sach_nhom_Tivi
    ))
    N_Tivi = Danh_sach[0] if len(Danh_sach)==1 else None
    return N_Tivi
